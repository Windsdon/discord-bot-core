...
DiscordBot.prototype.onMessage = function (user, userID, channelID, message, rawEvent) {
    ...
    Fiber(function() {
        ...
        async.applyEach(self.commandHandlers["start"], o, function(err, results) {
            if(err) {
                logger.info("Execution blocked");
                if(err.message && !err.silent) {
                    self.queueMessage(o.channelID, err.message);
        ...
        // check whitelist
        if(self.whitelist.indexOf(o.channelID) == -1) {
            if(o.obj.command.options.enableAll) {
        ...
        if(o.obj.params === false) {
            // wrong usage
            o.e.mention().text("Usage:").n().respond(obj.command.getHelp(self.parsers[serverID].activator));
            return;
        }
...
DiscordBot.prototype.getOutbound = function (channelID) {
    ...
    if(!this.outbound[channelID]) {
        this.outbound[channelID] = async.queue(function(task, callback) {
            if(task.message) {
