...
*/
function PermissionManager(dba, callback) {
    this.dba = dba;
...
PermissionManager.prototype.userHasGroup = function (group, uid) {
    if(typeof(group) == "string") {
        group = this.getGroup(group);
    }

    if(group && this.userCache[uid] && this.userCache[uid].groups.indexOf(group.gid) != -1) {
        return true;
    }
...
PermissionManager.prototype.getGroup = function (group) {
    var gid = this.getGID(group);
...
PermissionManager.prototype.removeGroup = function (group, sid, callback) {
    ...
    if(!this.groupExists(group, sid)) {
        logger.warn("This group doesn't exist");
        if(typeof(callback) == "function") {
            callback(false);
        }
        return false;
...
PermissionManager.prototype.getUsersInGroup = function (group, sid) {
    ...
    for (var uid in this.userCache) {
        if (this.userCache.hasOwnProperty(uid)) {
